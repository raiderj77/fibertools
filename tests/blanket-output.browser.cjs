...
await fill({'blanket-custom-width':'41.0000000001'});assert.match(await text(),/164 stitches/);assert.match(await text(),/less than 0\.0001 in/);assert.equal(await p.getByRole('radio').count(),2);await p.getByRole('button',{name:'📋 Copy',exact:true}).click();assert.match(await p.evaluate(()=>window.__copied),/less than 0\.0001 in/);await p.emulateMedia({media:'print'});assert.match(await text(),/less than 0\.0001 in/);await p.emulateMedia({media:'screen'});
...
fs.writeFileSync('blanket-browser-evidence.json',JSON.stringify({base:process.env.BLANKET_TEST_BASE || 'http://localhost:4031',passed:true,checks:['primary/alternative','dedup/exact/above','fractional ceiling','sub-display deficit screen/copy/print','keyboard','copy selected','print selected','A-B-A reset','yarn unchanged','unit equivalents','orphan/zero validation','mobile','no input POST/storage preference','reload'],requests:requests.map(r=>({method:r.method,origin:new URL(r.url).origin})),pageErrors:errors},null,2));console.log('Blanket browser acceptance checks passed.');
