...
| Runtime contract | Six focused assertions require Node 24 across the executing build, application/lockfile, all Node-running workflows, the security gate and development prerequisites. The observed Vercel major is already 24; this does not change provider runtime settings or dependency versions. Hosted CI exposed the obsolete engine assertion before merge; the security gate now requires the reviewed 24.x engine and matching .nvmrc without removing its other checks. |
