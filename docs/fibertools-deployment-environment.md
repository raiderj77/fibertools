...
| `PLANNING_PACK_PRIVATE_DELIVERY_CONFIRMED` | Server-only owner gate | Keep `false` or unset until a new sellable revision is in approved private delivery storage. |
| `PLANNING_PACK_PRIVATE_UPLOAD_CONFIRMED` | Server-only owner gate | Keep `false` until the exact checksum-matched private artifact is uploaded and retrieved successfully. |
