...
- `PLANNING_PACK_PRIVATE_UPLOAD_CONFIRMED=true` — server-only upload-placement confirmation. Set it only after the exact object name, MIME type, expected size, and non-public bucket are verified; it does not attest delivery or checksum retrieval.
- `PLANNING_PACK_PRIVATE_DELIVERY_CONFIRMED=true` — server-only delivery confirmation. **Keep unset or false until an exact paid non-customer session retrieves the checksum-matched revision through the protected route.**
...
- `config/planning-pack-release-manifest.json` records the edition, expected checksum, and verified private upload while delivery and owner verification remain `PENDING` and checkout remains `DISABLED`. Both the verifier and the runtime page block public checkout until edition, checksum, upload, delivery, enabled release states, checkout URL, and owner attestations agree.
