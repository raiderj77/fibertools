...
Only the random project ID and recovery key cross the first-party payment API boundary. The server stores a hash of the key, payment references, and processing state in private purchase records. It does not store the raw key, pattern, title, nickname, corrections, notes, calculated stitch values, or report. Stripe receives opaque purchase references, not local project contents. Browser analytics carry only the existing fixed consented events; purchase references and keys are excluded.
...
- New Checkout must use the exact approved active one-time USD 900-cent Price, quantity one, immediate card payment only, and fixed first-party return destinations. Owner tax configuration is a separate verification gate; do not infer tax registration or classification.
...
- APIs must bound input, accept only the exact credential fields, enforce same-origin browser requests, return generic errors, and use no-store/no-referrer/noindex headers. Credentials and provider payloads must not be logged.
...
The active Stripe Product must carry `service=stitchproof_designer_project` and `offer_version=STITCHPROOF-PROJECT-V1` metadata. Its exact active Price must be one-time USD 900 cents and match the configured Product/Price IDs and owner-confirmed tax behavior. The application creates project-bound Checkout Sessions; a reusable Payment Link is not a substitute for this binding. Currency conversion, quantity adjustment, promotions, phone collection, and Stripe-generated recovery checkouts are explicitly disabled.
