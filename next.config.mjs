...
const standardPageSecurityHeaders = [
  {
    key: 'X-Frame-Options',
    value: 'SAMEORIGIN',
  },
...
const embedPageSecurityHeaders = [
  {
    key: 'Content-Security-Policy',
    value: "default-src 'self'; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline'; img-src 'self' data:; font-src 'self'; connect-src 'self'; object-src 'none'; base-uri 'none'; form-action 'none'; frame-src 'none'; worker-src 'none'; frame-ancestors 'self' https:",
  },
  {
    key: 'X-Robots-Tag',
...
const nextConfig = {
  ...
  async headers() {
    return [
      ...
      {
        source: '/((?!embed(?:/|$)).*)',
        headers: standardPageSecurityHeaders,
      },
      {
        source: '/embed/:path*',
